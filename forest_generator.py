...
class Forest:
    ...
    def __init__(self, center: (int, int), tree_type: str):
        if tree_type not in self.forest_types:
            raise AttributeError(str(tree_type) + ' is not forest type: they are ' +
...
class ForestMap:
    ...
    def draw(self, painter: QPainter, scale: float = 10) -> None:
        colors = {'oak': QColor('#7E2E0F'),
                  'birch': QColor('white'),
                  'spruce': QColor('darkgreen')}
        pen = QPen()
        pen.setWidth(.75 * scale)
        for tree in self.trees_data:
            pen.setColor(colors[nearestForest(
                tree, self.forests)[0].tree_type])
            painter.setPen(pen)
...
def nearestForest(point: (float, float), forests: [Forest, ...],
                  metrics: FunctionType = manhattanMetric) -> (Forest, float):
...
def generateForestMap(width: float, height: float, density: float = 8) -> ForestMap:
    ...
    for x in range(0, ceil(width), 2):
        for y in range(0, ceil(height), 2):
            ...
            if abs(gauss(0, sigma)) < 1 / (distance + .7):
                trees.append((x + gauss(1, .9) + .1, y + gauss(1, .9) + .1))
...
def generateForests(width: float, height: float) -> [Forest, ...]:
    ...
    for i in range(ceil(width / 16)):
        for j in range(ceil(height / 16)):
            forests.append(Forest((randint(16 * i + 1, 16 * i + 15),
...
def drawGrass(painter: QPainter, width: float, height: float) -> None:
    ...
    painter.drawTiledPixmap(-grass_pixmap.width() / 2, -grass_pixmap.height() / 2,
                            width + grass_pixmap.width(), height + grass_pixmap.height(),
                            grass_pixmap)
